...
<!--
     change ltProblem to match the problem id that is 1 more than what
     we want for division1 (condition: &lt; $ltProblem)
  -->
<xsl:variable name="ltProblem" select="'3'" />
...
<a href="index2.html">Goto Division 2 Standings</a>
		    <br/>
		    <br/>
<a href="pacnw_div1.pdf">Division 1 Problem Descriptions</a>
		    &#160;
		    &#160;
...
<td><xsl:value-of select="@totalAttempts"/>/<xsl:value-of select="@totalSolved"/></td>
        ...
        <xsl:template name="problemsummary">
            <xsl:for-each select="/contestStandings/standingsHeader/problem[@id &lt; $ltProblem]">
...
<xsl:value-of select="@totalAttempts"/>/<xsl:value-of select="@solved"/></td>
        ...
        <xsl:template name="problemSummaryInfo">
            <xsl:for-each select="problemSummaryInfo[@index &lt; $ltProblem]">
...
<th>&#160;&#160;&#160;&#160;<strong><u><xsl:number format="A" value="@id"/></u></strong>&#160;&#160;&#160;&#160;</th>
	    ...
	    <xsl:for-each select="/contestStandings/standingsHeader/colorList/colors[@siteNum = 1]/problem[@id &lt; $ltProblem]">
