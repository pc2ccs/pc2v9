...
<!--
     ...
     than we want for division3 (condition: &gt; $gtProblem)
     change gtGroupId to match the number of pacnw groups
  -->
<xsl:variable name="gtProblem" select="'6'" />
<xsl:variable name="gtGroupId" select="'10'" />
...
<META HTTP-EQUIV="PRAGMA" CONTENT="NO-CACHE"/>
	...
	<BODY>
		...
		<center>
<IMG  style="width: 1200px; height: 200px;" SRC="regional-header-RM.png" align="center"/>
		    <h2>Rocky Mountain Regional Contest<br/></h2>
		    <h3></h3>
		    &#160;
                  <br/>
...
<xsl:if test="@id > $gtGroupId">
		    ...
		    <br/>
		    <br/>
<a href="div3problems.html">RM Problem Descriptions</a>
		    &#160;
		    &#160;

<a href="sampled3.zip">Sample RM Data</a>
		    &#160;
		    &#160;
...
<xsl:if test="@bestSolutionTime"><xsl:value-of select="@bestSolutionTime"/></xsl:if>/<xsl:value-of select="@numberSolved"/>
        ...
        <xsl:template name="teamStanding">
            <xsl:for-each select="teamStanding[@teamGroupId > $gtGroupId]">
...
<xsl:if test="@isSolved = 'true'"><xsl:value-of select="@solutionTime"/></xsl:if>
        ...
        <xsl:template name="problemTitle">
            <xsl:for-each select="/contestStandings/standingsHeader/problem[@id &gt; $gtProblem]">
<xsl:comment>
TODO todo update this 4 is gtProblem changes
</xsl:comment>
<th>&#160;&#160;&#160;&#160;<strong><u><xsl:number format="A" value="@id - 4"/></u></strong>&#160;&#160;&#160;&#160;</th>
            </xsl:for-each>
        </xsl:template>
