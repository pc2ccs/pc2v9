...
<!--
     ...
     and
     change lteqProblem to match the problem id that 
     than we want for division2 (condition: &lt;= $lteqProblem)
     change lteqGroupId to be what we have for pacnw sites
     (D1 and D2 combined)
  -->
<xsl:variable name="gtProblem" select="'3'" />
<xsl:variable name="lteqProblem" select="'6'" />
...
<a href="index1.html">Goto Division 1 Standings</a>
		    ...
		    <br/>
		    <br/>
<a href="div2problems.html">Division 2 Problem Descriptions</a>
		    &#160;
		    &#160;
...
<td><xsl:value-of select="@totalAttempts"/>/<xsl:value-of select="@totalSolved"/></td>
        ...
        <xsl:template name="problemsummary">
            <xsl:for-each select="/contestStandings/standingsHeader/problem[@id &gt; $gtProblem and @id &lt;= $lteqProblem]">
...
<xsl:if test="@teamGroupId &lt;= $lteqGroupId">
  <xsl:if test="@teamGroupId mod 2 = 0">
...
<xsl:value-of select="@totalAttempts"/>/<xsl:value-of select="@solved"/></td>
        ...
        <xsl:template name="problemSummaryInfo">
            <xsl:for-each select="problemSummaryInfo[@index &gt; $gtProblem and @id &lt;= $lteqProblem]">
...
<xsl:if test="@isSolved = 'true'"><xsl:value-of select="@solutionTime"/></xsl:if>
        ...
        <xsl:template name="problemTitle">
            <xsl:for-each select="/contestStandings/standingsHeader/problem[@id &gt; $gtProblem and @id &lt;= $lteqProblem]">
<xsl:comment>
TODO todo update this 2 is gtProblem changes
</xsl:comment>
<th>&#160;&#160;&#160;&#160;<strong><u><xsl:number format="A" value="@id - 2"/></u></strong>&#160;&#160;&#160;&#160;</th>
            </xsl:for-each>
        </xsl:template>
        <xsl:template name="problemColor">
	    <xsl:for-each select="/contestStandings/standingsHeader/colorList/colors[@siteNum = 1]/problem[@id &gt; $gtProblem and @id &lt;= $lteqProblem]">
