...
b.tablesorter.addWidget({id:"filter",format:function(c){if(c.config.parsers&&!b(c).hasClass("hasFilters")){var a,d,e,g,f,l,n,m,q,h,r,w,t,k,u,s,p,F,z,E=b.tablesorter.formatFloat,K="",B=b.tablesorter,v=c.config,C=b(v.headerList),j=v.widgetOptions, D=j.filter_cssFilter||"tablesorter-filter",y=b(c).addClass("hasFilters"),J=v.$tbodies,I=v.parsers.length,L=/^\/((?:\\\/|[^\/])+)\/([mig]{0,3})?$/,T=RegExp(v.cssChildRow),U=/undefined|number/,M=/(^[\"|\'|=])|([\"|\'|=]$)/g,G=/[^\w,. \-()]/g,V=/[<>=]/g,N,O,P,H=function(a){var c=b.isArray(a),d=y.find("thead").eq(0).find(".tablesorter-filter-row").children(),e=c?a:d.map(function(a){a=b(this).find("select."+D+", input."+D);return a.length?a.val()||"":""}).get(),f=(e||[]).join("");c&&d.each(function(c, d){b(d).val(a[c]||"")});!0===j.filter_hideFilters&&y.find(".tablesorter-filter-row").trigger(""===f?"mouseleave":"mouseenter");if(!(K===f&&!1!==a))if(y.trigger("filterStart",[e]),v.showProcessing)setTimeout(function(){Q(a,e,f);return!1},30);else return Q(a,e,f),!1},Q=function(k,g,l){var h,r,q,t,w,x,F,A,z;v.debug&&(F=new Date);for(e=0;e<J.length;e++){k=b.tablesorter.processTbody(c,J.eq(e),!0);h=k.children("tr");w=h.length;if(""===l||j.filter_serversideFiltering)h.show().removeClass("filtered");else for(d= 0;d<w;d++)if(!T.test(h[d].className)){t=!0;q=h.eq(d).nextUntil("tr:not(."+v.cssChildRow+")");p=q.length&&(j&&j.hasOwnProperty("filter_childRows")&&"undefined"!==typeof j.filter_childRows?j.filter_childRows:1)?q.text():"";p=j.filter_ignoreCase?p.toLocaleLowerCase():p;r=h.eq(d).children("td");for(a=0;a<I;a++)if(g[a]){n=j.filter_useParsedData||N[a]?v.cache[e].normalized[d][a]:b.trim(r.eq(a).text());m=!U.test(typeof n)&&j.filter_ignoreCase?n.toLocaleLowerCase():n;x=t;f=j.filter_ignoreCase?g[a].toLocaleLowerCase(): g[a];if(j.filter_functions&&j.filter_functions[a])!0===j.filter_functions[a]?x=C.filter('[data-column="'+a+'"]:last').hasClass("filter-match")?0<=m.search(f):g[a]===n:"function"===typeof j.filter_functions[a]?x=j.filter_functions[a](n,v.cache[e].normalized[d][a],g[a],a):"function"===typeof j.filter_functions[a][g[a]]&&(x=j.filter_functions[a][g[a]](n,v.cache[e].normalized[d][a],g[a],a));else if(L.test(f)){u=L.exec(f);try{x=RegExp(u[1],u[2]).test(m)}catch(D){x=!1}}else if(M.test(f)&&m==f.replace(M, ""))x=!0;else if(/^\!/.test(f))f=f.replace("!",""),s=m.search(b.trim(f)),x=""===f?!0:!(j.filter_startsWith?0===s:0<=s);else if(/^[<>]=?/.test(f))u=isNaN(m)?E(m.replace(G,""),c):E(m,c),s=E(f.replace(G,"").replace(V,""),c),/>/.test(f)&&(x=/>=/.test(f)?u>=s:u>s),/</.test(f)&&(x=/<=/.test(f)?u<=s:u<s),""===s&&(x=!0);else if(/\s+(AND|&&)\s+/g.test(g[a])){s=f.split(/(?:\s+(?:and|&&)\s+)/g);x=0<=m.search(b.trim(s[0]));for(A=s.length-1;x&&A;)x=x&&0<=m.search(b.trim(s[A])),A--}else/\s+(-|to)\s+/.test(f)?(u= isNaN(m)?E(m.replace(G,""),c):E(m,c),s=f.split(/(?: - | to )/),A=E(s[0].replace(G,""),c),z=E(s[1].replace(G,""),c),A>z&&(x=A,A=z,z=x),x=u>=A&&u<=z||""===A||""===z?!0:!1):/[\?|\*]/.test(f)||/\s+OR\s+/.test(g[a])?x=RegExp(f.replace(/\s+or\s+/gi,"|").replace(/\?/g,"\\S{1}").replace(/\*/g,"\\S*")).test(m):(n=(m+p).indexOf(f),x=!j.filter_startsWith&&0<=n||j.filter_startsWith&&0===n);t=x?t?!0:!1:!1}h[d].style.display=t?"":"none";h.eq(d)[t?"removeClass":"addClass"]("filtered");if(q.length)q[t?"show":"hide"]()}b.tablesorter.processTbody(c, k,!1)}K=l;v.debug&&B.benchmark("Completed filter widget search",F);y.trigger("applyWidgets");y.trigger("filterEnd")},R=function(a,f){var k,h=[];a=parseInt(a,10);k='<option value="">'+(C.filter('[data-column="'+a+'"]:last').attr("data-placeholder")||"")+"</option>";for(e=0;e<J.length;e++){g=v.cache[e].row.length;for(d=0;d<g;d++)j.filter_useParsedData?h.push(""+v.cache[e].normalized[d][a]):(p=v.cache[e].row[d][0].cells[a])&&h.push(b.trim(v.supportsTextContent?p.textContent:b(p).text()))}h=b.grep(h, function(a,c){return b.inArray(a,h)===c});h=B.sortText?h.sort(function(b,d){return B.sortText(c,b,d,a)}):h.sort(!0);for(e=0;e<h.length;e++)k+=""!==h[e]?'<option value="'+h[e].replace(/\"/g,"&quot;")+'">'+h[e]+"</option>":"";y.find("thead").find("select."+D+'[data-column="'+a+'"]')[f?"html":"append"](k)},S=function(b){for(a=0;a<I;a++)if(p=C.filter('[data-column="'+a+'"]:last'),(p.hasClass("filter-select")||j.filter_functions&&!0===j.filter_functions[a])&&!p.hasClass("filter-false"))j.filter_functions|| (j.filter_functions={}),j.filter_functions[a]=!0,R(a,b)};v.debug&&(O=new Date);j.filter_ignoreCase=!1!==j.filter_ignoreCase;j.filter_useParsedData=!0===j.filter_useParsedData;if(!1!==j.filter_columnFilters&&C.filter(".filter-false").length!==C.length){p='<tr class="tablesorter-filter-row">';for(a=0;a<I;a++)p+="<td></td>";k=y.find("thead").eq(0).append(p+="</tr>").find("td");for(a=0;a<I;a++)F=!1,t=C.filter('[data-column="'+a+'"]:last'),l=j.filter_functions&&j.filter_functions[a]&&"function"!==typeof j.filter_functions[a]|| t.hasClass("filter-select"),F=B.getData?"false"===B.getData(t[0],v.headers[a],"filter"):v.headers[a]&&v.headers[a].hasOwnProperty("filter")&&!1===v.headers[a].filter||t.hasClass("filter-false"),l?p=b("<select>").appendTo(k.eq(a)):(j.filter_formatter&&b.isFunction(j.filter_formatter[a])?((p=j.filter_formatter[a](k.eq(a),a))&&0===p.length&&(p=k.eq(a).children("input")),p&&(0===p.parent().length||p.parent().length&&p.parent()[0]!==k[a])&&k.eq(a).append(p)):p=b('<input type="search">').appendTo(k.eq(a)), p&&p.attr("placeholder",t.attr("data-placeholder")||"")),p&&(p.addClass(D).attr("data-column",a),F&&(p.addClass("disabled")[0].disabled=!0))}y.bind("addRows updateCell update updateRows appendCache filterReset search ".split(" ").join(".tsfilter "),function(a,b){/(search|filterReset)/.test(a.type)||S(!0);"filterReset"===a.type&&y.find("."+D).val("");H("search"===a.type?b:"");return!1}).find("input."+D).bind("keyup search",function(a,b){if(!(32>a.which&&8!==a.which||37<=a.which&&40>=a.which)){if("undefined"!== typeof b)return H(b),!1;clearTimeout(P);P=setTimeout(function(){H()},j.filter_searchDelay||300)}});N=C.map(function(a){return B.getData?"parsed"===B.getData(C.filter('[data-column="'+a+'"]:last'),v.headers[a],"filter"):b(this).hasClass("filter-parsed")}).get();j.filter_reset&&b(j.filter_reset).length&&b(j.filter_reset).bind("click.tsfilter",function(){y.trigger("filterReset")});if(j.filter_functions)for(z in j.filter_functions)if(j.filter_functions.hasOwnProperty(z)&&"string"===typeof z)if(p=C.filter('[data-column="'+ z+'"]:last'),l="",!0===j.filter_functions[z]&&!p.hasClass("filter-false"))R(z);else if("string"===typeof z&&!p.hasClass("filter-false")){for(h in j.filter_functions[z])"string"===typeof h&&(l+=""===l?'<option value="">'+(p.attr("data-placeholder")||"")+"</option>":"",l+='<option value="'+h+'">'+h+"</option>");y.find("thead").find("select."+D+'[data-column="'+z+'"]').append(l)}S(!0);y.find("select."+D).bind("change search",function(){H()});!0===j.filter_hideFilters&&y.find(".tablesorter-filter-row").addClass("hideme").bind("mouseenter mouseleave", function(a){var c;r=b(this);clearTimeout(q);q=setTimeout(function(){/enter|over/.test(a.type)?r.removeClass("hideme"):b(document.activeElement).closest("tr")[0]!==r[0]&&(c=y.find("."+(j.filter_cssFilter||"tablesorter-filter")).map(function(){return b(this).val()||""}).get().join(""),""===c&&r.addClass("hideme"))},200)}).find("input, select").bind("focus blur",function(a){w=b(this).closest("tr");clearTimeout(q);q=setTimeout(function(){if(""===y.find("."+(j.filter_cssFilter||"tablesorter-filter")).map(function(){return b(this).val()|| ""}).get().join(""))w["focus"===a.type?"removeClass":"addClass"]("hideme")},200)});v.showProcessing&&y.bind("filterStart.tsfilter filterEnd.tsfilter",function(a,c){var d=c?y.find("."+v.cssHeader).filter("[data-column]").filter(function(){return""!==c[b(this).data("column")]}):"";B.isProcessing(y[0],"filterStart"===a.type,c?d:"")});v.debug&&B.benchmark("Applying Filter widget",O);y.trigger("filterInit");H()}},remove:function(c,a,d){var e,g;e=b(c);a=a.$tbodies;e.removeClass("hasFilters").unbind("addRows updateCell update appendCache search filterStart filterEnd ".split(" ").join(".tsfilter ")).find(".tablesorter-filter-row").remove(); for(e=0;e<a.length;e++)g=b.tablesorter.processTbody(c,a.eq(e),!0),g.children().removeClass("filtered").show(),b.tablesorter.processTbody(c,g,!1);d.filterreset&&b(d.filter_reset).unbind("click.tsfilter")}});
