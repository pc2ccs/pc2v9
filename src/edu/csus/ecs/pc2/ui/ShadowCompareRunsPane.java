    ...
    protected boolean updateRun(Integer submissionId, CLICS_JUDGEMENT_ACRONYM newJudgement) {
        ...
        for (Judgement judgement : judgementsArray) {
            ...
            if (newJudgement.name().contentEquals(judgement.getAcronym())) {
                ...
                // update judging time (that is, how long did it take to get a response back from the remote after submission received)
                TimeZone tz = TimeZone.getTimeZone("GMT");
                GregorianCalendar cal = new GregorianCalendar(tz);

                long milliDiff = cal.getTime().getTime() - targetRun.getCreateDate().getTime();
                long totalSeconds = milliDiff / 1000;
                judgementRecord.setHowLongToJudgeInSeconds(totalSeconds);
                judgementRecord.setJudgeStartDate(targetRun.getCreateDate());
