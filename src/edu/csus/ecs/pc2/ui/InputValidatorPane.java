...
public class InputValidatorPane extends JPanePlugin {
     ...
     * 
     * The worker thread publishes each separate InputValidationResult as it is generated; each published result is automatically picked up and handled by the worker's process() method. Once the
     * worker thread completes it assigns the results to a global variable which is accessible by external clients via an accessor.
     * 
    ...
    private void spawnInputValidatorRunnerThread() {
        ...
        SwingWorker<InputValidationResult[], InputValidationResult> worker = new SwingWorker<InputValidationResult[], InputValidationResult>() {
            ...
            /**
             * Method doInBackground() is invoked when the Worker thread's execute() method is called (which happens below, after the worker thread has been constructed). The method runs the Input
             * Validator in the background against all the judge's input data files currently defined on the EditProblemPane's Input Data Files pane, publishing each result as it finishes. When the
             * method is finished it returns an array of all InputValidationResults; this array is accessible by the Worker thread's done() method (via a call to get()).
             */
            @Override
...
//                    }
                    ...
                    getController().getLog().warning("Attempted to return Input Validator results to a " + parent.getClass() + "; currently only EditProblemPane is supported for receiving results");
            ...
            /**
             * This method is invoked by the Worker thread when it is completely finished with its doInBackground task(s). Calling get() fetches the set of data returned by the Worker thread's
             * doInBackground() method -- that is, an array of InputValidationResults. This method saves those results so they can be accessed by external code via the {@link #getRunResults()} method.
    ...
    /**
     * This method is called when spawnInputValidatorRunnerThread() finishes. Its job is to restore the GUI state, including resetting the cursor and reenabling the RunInputValidator button.
     */
    ...
    /**
     * Updates the Input Validator Run Status flag in this InputValidatorPane. Note that this does NOT update the Problem; just the local status flag. The Problem only gets updated when Add or Update
    ...
    /**
     * Returns the {@link InputValidationStatus} for this {@link InputValidatorPane}. The returned value indicates whether an Input Validator has been run, and if so whether all runs passed or not.
     * 
     ...
     * 
     * @param seqNum
     *            a sequence number for associating with generated file names
     * 
     * @param problem
     *            the Contest Problem associated with the Input Validator
     * @param validatorProg
     *            the Input Validator Program to be run
     * @param validatorCommand
     *            the command used to run the Input Validator Program
     * @param dataFile
     *            the data file to be passed to the Input Validator as input to be validated
     * @param executeDir
     *            the execution directory to be used (in which to run the Input Validator Program
     * 
     * @return an InputValidationResult
     * 
     * @throws ExecutionException
     *             if an ExecutionException occurs during execution of the Input Validator
     * @throws Exception
     *             if an Exception other than ExecutionException occurs during execution of the Input Validator
     */
