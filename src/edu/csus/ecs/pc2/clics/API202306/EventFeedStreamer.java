...
public class EventFeedStreamer extends JSON202306Utilities implements Runnable, UIPlugin, IEventFeedStreamer {
    ...
    protected class ClarificationListener implements IClarificationListener {
        ...
        public void clarificationAdded(ClarificationEvent event) {
            ...
            String json = getJSONEvent(CLARIFICATIONS_KEY, getNextEventId(), IJSONTool.getClarificationId(clarification), jsonTool.convertToJSON(clarification, null).toString());
            sendJSON(json + NL);
        }
        ...
        public void clarificationChanged(ClarificationEvent event) {
            ...
            String json = getJSONEvent(CLARIFICATIONS_KEY, getNextEventId(), IJSONTool.getClarificationId(clarification), jsonTool.convertToJSON(clarification, clarAnswers[clarAnswers.length - 1]).toString());
            sendJSON(json + NL);
        }
        ...
        public void clarificationRemoved(ClarificationEvent event) {
            ...
            String json = getJSONEvent(CLARIFICATIONS_KEY, getNextEventId(), IJSONTool.getClarificationId(clarification), "{\"id\": \"" + id + "\"}");
            sendJSON(json + NL);
            ClarificationAnswer[] clarAnswers = clarification.getClarificationAnswers();
            if (clarAnswers != null) {
    ...
    protected class ProblemListener implements IProblemListener {
        ...
        public void problemAdded(ProblemEvent event) {
            ...
            String json = getJSONEvent(PROBLEM_KEY, getNextEventId(), IJSONTool.getProblemId(problem), jsonTool.convertToJSON(problem, problemNumber).toString());
            sendJSON(json + NL);
        }
    ...
    protected class LanguageListener implements ILanguageListener {
        ...
        public void languageAdded(LanguageEvent event) {
            ...
            String json = getJSONEvent(LANGUAGE_KEY, getNextEventId(), IJSONTool.getLanguageId(language), jsonTool.convertToJSON(language).toString());
            sendJSON(json + NL);
        }
    ...
    protected class GroupListener implements IGroupListener {
        ...
        public void groupAdded(GroupEvent event) {
            ...
            String json = getJSONEvent(GROUPS_KEY, getNextEventId(), IJSONTool.getGroupId(group), jsonTool.convertToJSON(group).toString());
            sendJSON(json + NL);
    ...
    protected class JudgementListener implements IJudgementListener {
        ...
        public void judgementAdded(JudgementEvent event) {
            ...
            String json = getJSONEvent(JUDGEMENT_TYPE_KEY, getNextEventId(), judgement.getAcronym(), jsonTool.convertToJSON(judgement).toString());
            sendJSON(json + NL);
        }
        ...
        public void judgementChanged(JudgementEvent event) {
            ...
            if (judgement.isActive()) {
                json = getJSONEvent(JUDGEMENT_TYPE_KEY, getNextEventId(), judgement.getAcronym(), jsonTool.convertToJSON(judgement).toString());
            } else {
                json = getJSONEvent(JUDGEMENT_TYPE_KEY, getNextEventId(), judgement.getAcronym(), jsonTool.convertToJSON(judgement).toString());
            }
            sendJSON(json + NL);
    ...
    protected class ContestInformationListener implements IContestInformationListener {
        ...
        public void contestInformationRemoved(ContestInformationEvent event) {
            // JB - Not supported by the API, for CONTEST_KEY, id must always be null anyway.
//            String json = getJSONEvent(CONTEST_KEY, getNextEventId(), null /*EventFeedOperation.DELETE*/, "{\"id\": \"" + contest.getContestIdentifier() + "\"}");
//            sendJSON(json + NL);
        }
    ...
    private String replaceEventId(String string, long newId) {
        ...
        // {"type":"languages", "token":"-1",
        // {"type":"languages", "token":"pc2-14",

        String newString =string.replaceFirst("\"token\":\"-1\"","\"token\":\""+ EventFeedJSON.getEventId(newId)+"\"");
        return newString.replaceFirst("\"token\":\"pc2--1\"","\"token\":\"" + EventFeedJSON.getEventId(newId)+"\"");
    }
    ...
    /**
     * Create a snap shot of the 2023-06 JSON event feed.
     * This is called directly by JUnit tests: services.web.EventFeedStreamerTest test unit.
     */
